...
module.exports = {
    ...
    create: (data, callback) => {
        // Authentication: any
        // Required Parameters: name, summary, description, rules
        // Optional Parameters: none

        if(!data.auth) return callback('auth is required');
        if(!data.name) return callback('name is required');
        if(!data.summary) return callback('summary is required');
        if(!data.description) return callback('description is required');
        if(!data.rules) return callback('rules is required');

        let options = {};
        options.url = `https://api.twitch.tv/kraken/communities`;
        options.form = {
            name: data.name,
    ...
    update: (data, callback) => {
        // Authentication: any (community owner)
        // Required Parameters: communityID
    ...
    bans: (data, callback) => {
        // Authentication: any (community owner or moderator)
        // Required Parameters: communityID
        // Optional Parameters: limit, cursor
        ...
        let options = {};
        options.url = `https://api.twitch.tv/kraken/communities/${data.communityID}/bans`;
    ...
    addTimeout: (data, callback) => {
        ...
        // Optional Parameters: reason, limit, cursor
